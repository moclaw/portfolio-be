...
func Load() *Config {
	...
	config := &Config{
		...
		S3Config: S3Config{
			Endpoint:        getSecretOrEnv(secretData, "s3_endpoint", "S3_ENDPOINT", "http://localhost:4566"),
			Region:          getSecretOrEnv(secretData, "s3_region", "S3_REGION", "us-east-1"),
			Bucket:          getSecretOrEnv(secretData, "s3_bucket", "S3_BUCKET", "portfolio-bucket"),
			AccessKeyID:     getSecretOrEnv(secretData, "s3_access_key_id", "S3_ACCESS_KEY_ID", "test"),
			SecretAccessKey: getSecretOrEnv(secretData, "s3_secret_access_key", "S3_SECRET_ACCESS_KEY", "test"),
			ForcePathStyle:  true,
		},
		JWTConfig: JWTConfig{
